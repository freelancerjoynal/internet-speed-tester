...
export default function WikimediaBandwidthTest() {
  const [downloadedMB, setDownloadedMB] = useState<string>('0.00');
  const [downloadedGB, setDownloadedGB] = useState<string>('0.000');
  ...
  const [connectionQuality, setConnectionQuality] = useState<string>('📶 Connecting...');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem('wikimedia_test_data');
        if (saved) {
          const data = JSON.parse(saved);
          totalBytesRef.current = data.totalBytes || 0;
          setResetCount(data.resetCount || 0);
          maxSpeedRef.current = data.maxSpeed || 0;
          setMaxSpeed(maxSpeedRef.current.toFixed(2));
          minSpeedRef.current = data.minSpeed || Infinity;
          setMinSpeed(minSpeedRef.current === Infinity ? '0.00' : minSpeedRef.current.toFixed(2));
          totalElapsedSecondsRef.current = data.totalElapsedSeconds || 0;
          ...
          const pct = Math.min((mb / (100 * 1024)) * 100, 100);
          setProgress(pct);
        }
      } catch(e) {}
  ...
  const saveData = () => {
    try {
      localStorage.setItem('wikimedia_test_data', JSON.stringify({
        totalBytes: totalBytesRef.current,
        resetCount: resetCount,
        maxSpeed: maxSpeedRef.current,
        minSpeed: minSpeedRef.current,
  ...
  useEffect(() => {
    ...
    const speedInterval = setInterval(() => {
      ...
      if (duration > 0 && isRunningRef.current) {
        ...
        if (speedMbps > 0) {
          ...
          if (speedHistoryRef.current.length > 0) {
            ...
            else if (avg > 0) setConnectionQuality('🐌 Slow');
          }
        }
        ...
        // Save periodically
        if (Math.floor(totalBytesRef.current / (1024 * 1024)) % 5 === 0) {
          saveData();
        }
  ...
  const handleResetData = () => {
    if (confirm('Reset all stats?')) {
      localStorage.removeItem('wikimedia_test_data');
  ...
  return (
    <main className="min-h-screen w-full bg-gradient-to-br from-indigo-900 via-purple-900 via-pink-900 to-rose-900 flex items-center justify-center p-0 sm:p-4 md:p-6">
      ...
      <div className="w-full max-w-4xl relative z-10">
        <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-none sm:rounded-3xl p-4 sm:p-6 md:p-8 shadow-2xl shadow-purple-500/20 min-h-screen sm:min-h-0">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-white/10 pb-4 mb-4 sm:mb-6 gap-3">
            <div className="flex items-center gap-3 w-full sm:w-auto">
              ...
              <div className="min-w-0 flex-1">
                <h1 className="text-lg sm:text-xl md:text-2xl font-bold bg-gradient-to-r from-pink-400 via-purple-400 via-cyan-400 to-emerald-400 bg-clip-text text-transparent">
                  Wikimedia CDN Test
                </h1>
                <p className="text-xs text-slate-400 flex flex-wrap items-center gap-1 sm:gap-2">
                  ...
                  <span className="text-xs">CDN Bandwidth</span>
                </p>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-br from-slate-900/80 via-slate-800/80 to-slate-900/80 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-white/10 text-center mb-4 sm:mb-6">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-center gap-2 sm:gap-4 text-xs sm:text-sm">
              ...
              <div className="flex items-center gap-1 sm:gap-2 bg-gradient-to-r from-rose-500/10 to-red-500/10 px-2 sm:px-3 py-1 rounded-full border border-rose-500/20">
                ...
                <span className="text-slate-400">Min: <span className="text-rose-400 font-bold">{minSpeed}</span></span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mb-4 sm:mb-6">
            ...
            <div className="bg-gradient-to-br from-rose-500/20 via-red-500/20 to-pink-500/20 p-3 sm:p-4 rounded-xl border border-rose-500/30 hover:scale-105 transition-transform">
              ...
              <div className="text-xl sm:text-2xl font-bold text-rose-400 mt-1">
                {minSpeed} <span className="text-sm font-normal text-slate-400">Mbps</span>
              </div>
          ...
          <div className="text-center text-[10px] sm:text-xs text-slate-500 border-t border-white/10 pt-4 flex flex-wrap items-center justify-center gap-2 sm:gap-4">
            ...
            <span className="flex items-center gap-1">
              ...
              <span className="hidden xs:inline">Wikimedia CDN</span>
              <span className="xs:hidden">CDN</span>
            </span>
          </div>
        </div>
      </div>
